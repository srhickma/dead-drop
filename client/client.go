...
func setupKeyFlags(cmd *cobra.Command) {
...
func setupAddKeyCmd() *cobra.Command {
	cmdAddKey := &cobra.Command{
		Use:   "add-key <public key path> <key name> <remote>",
		Short: "Add a public key as an authorized key on remote",
		Args:  cobra.MinimumNArgs(3),
		Run: func(cmd *cobra.Command, args []string) {
			pubKeyPath := args[0]
			keyName := args[1]
			remote := strings.TrimSuffix(args[2], "/")

			bindKeyFlags(cmd)

			if err := addKey(pubKeyPath, keyName, remote); err != nil {
...
func addKey(pubKeyPath string, keyName string, remote string) error {
...
func makeAuthenticatedRequestInternal(client *http.Client, req *http.Request, remote string) (*http.Response, error) {
	keyName := viper.GetString(keyNameFlag)
	if keyName == "" {
		return nil, fmt.Errorf("key name not specified")
	}
